...
public class CharacteristicDetailsAdapter extends BaseAdapter {
	...
	public void newValueForCharacteristic(final BluetoothGattCharacteristic ch, final String strVal, final int intVal, final byte[] rawValue, final String timestamp) {
		...
		mIntValue = intVal;
		decryptor = Math.abs((Math.pow(mIntValue,7))%55);
		flagval = intVal;
		mStrValue = strVal;
		mRawValue = rawValue;
	...
	public View getView(int position, View convertView, ViewGroup p) {
        ...
        if (convertView == null) {
			...
			fields.decryptedVal = (TextView) convertView.findViewById(R.id.decrypted);
			fields.decrypted = (Button) convertView.findViewById(R.id.decrypt);
			...
			//------------------------------------------------------------------------------
			fields.decrypted.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					String apnaKey = String.valueOf(decryptor);
					int hexon = (int) decryptor;
					String hexor = Integer.toHexString(hexon);
					//Toast.makeText(v.getContext(),
							//"Decrypted key is : " + apnaKey+"which in Hex is "+hexor, Toast.LENGTH_LONG).show();
					fields.decryptedVal.setText(String.valueOf(decryptor)+"/ Ox"+hexor);
...
//
		...
		//fields.decryptedVal.setText(String.valueOf(decryptor));
